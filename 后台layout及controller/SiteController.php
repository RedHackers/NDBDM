<?php
/**
 * Team: red hackers
 * Coding by cuilixiao 1511366, 20170706
 * This is the main layout of backend web. 
 */

namespace backend\controllers;

use Yii;
use common\models\LoginForm;
use frontend\models\PasswordResetRequestForm;
use frontend\models\ResetPasswordForm;
use frontend\models\SignupForm;
use frontend\models\ContactForm;
use yii\base\InvalidParamException;
use yii\web\BadRequestHttpException;
use yii\web\Controller;
use yii\filters\VerbFilter;
use yii\filters\AccessControl;
use yii\helpers\Url;

/**
 * Site controller
 */
class SiteController extends Controller
{
    public $layout = "main_layout";//cookie和session信息
    //public $layout = "main";
    /**
     * @inheritdoc
     */
    public function behaviors()
    {
        return [
            'access' => [
                'class' => AccessControl::className(),
                'only' => ['logout', 'signup'],
                'rules' => [
                    [
                        'actions' => ['signup'],
                        'allow' => true,
                        'roles' => ['?'],
                    ],
                    [
                        'actions' => ['logout'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
            ],
            'verbs' => [
                'class' => VerbFilter::className(),
                'actions' => [
                    'logout' => ['post'],
                ],
            ],
        ];
    }

    /**
     * @inheritdoc
     */
    public function actions()
    {
        return [
            'error' => [
                'class' => 'yii\web\ErrorAction',
            ],
            'captcha' => [
                'class' => 'yii\captcha\CaptchaAction',
                'fixedVerifyCode' => YII_ENV_TEST ? 'testme' : null,
            ],
        ];
    }

    /**
     * Displays homepage.
     *
     * @return mixed
     */ //$this->layout = "main_layout";
       
    public function actionIndex()
    {
        /*\YII::beginProfile('profile0');
        echo 'hello';
        \Yii::Warning('Hello, I am a test log message');
        \YII::endProfile('profile0');*/
        return $this->render('index');
    }

    /**
     * Logs in a user.
     *
     * @return mixed
     */
    public function actionLogin()
    {
       // $this->layout = "main";
        if (!\Yii::$app->user->isGuest) {
            return $this->goHome();
        }

        $model = new LoginForm();
        if ($model->load(Yii::$app->request->post()) && $model->login()) {
            return $this->goHome();
        } else {
            return $this->render('login', [
                'model' => $model,
            ]);
        }
    }

    /**
     * Logs out the current user.
     *
     * @return mixed
     */
    public function goLogin()
    {
        return Yii::$app->getResponse()->redirect(Url::to(['site/login']));
    }

    public function actionLogout()
    {
        Yii::$app->user->logout();
        
        return $this->goLogin();
    }

    /**
     * Displays contact page.
     *
     * @return mixed
     */
    public function actionContact()
    {
        $model = new ContactForm();
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail(Yii::$app->params['adminEmail'])) {
                Yii::$app->session->setFlash('success', 'Thank you for contacting us. We will respond to you as soon as possible.');
            } else {
                Yii::$app->session->setFlash('error', 'There was an error sending email.');
            }

            return $this->refresh();
        } else {
            return $this->render('contact', [
                'model' => $model,
            ]);
        }
    }

    /**
     * Displays about page.
     *
     * @return mixed
     */
    public function actiontAbout()
    {
        //$this->layout = "main2";
        return $this->render('about');
    }


    /** 
     * Displays teamwork page.
     */
    public function actionTeamwork()
    {
        return $this->render('teamwork');
    }//显示界面

     /** 
     * Displays personal page.
     */
    public function actionPersonalwork1()
    {
        return $this->render('personalwork1');
    }

      /** 
     * Displays personal page.
     */
    public function actionPersonalwork2()
    {
        return $this->render('personalwork2');
    }
    //显示界面
    public function actionPersonalwork3()
    {
        return $this->render('personalwork3');
    }
    //显示界面
     public function actionPersonalwork4()
    {
        return $this->render('personalwork4');
    }
    //显示界面
    /**
    **
    */
     public function actionTeam1()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-team-info/index'));
    }//显示界面

 public function actionTeam2()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-team-server/index'));
    }//显示界面
/**
*
*
*/
public function actionMemberwork1()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-member-info/index'));
    }//显示界面

 public function actionMemberwork2()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-member-live/index'));
    }//显示界面

    /**
*
*
*/
public function actionActivitywork1()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-activity-info/index'));
    }//显示界面

 public function actionActivitywork2()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-activity-attend/index'));
    }//显示界面

      /**
*
*
*/
public function actionNotework1()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-note-record/index'));
    }//显示界面

 public function actionNotework2()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-note-show/index'));
    }//显示界面



    public function actionLink()
    {
        //$control=Yii::app()->runController('RhTeamInfo/index');
        $this->redirect(array('rh-info-link/index'));
    }//显示界面

     /** 
     * Displays personal page.
     * Signs user up.
     *
     * @return mixed
     */
    public function actionSignup()
    {
        $model = new SignupForm();
        if ($model->load(Yii::$app->request->post())) {
            if ($user = $model->signup()) {
                if (Yii::$app->getUser()->login($user)) {
                    return $this->goHome();
                }
            }
        }

        return $this->render('signup', [
            'model' => $model,
        ]);
    }

    /**
     * Requests password reset.
     *
     * @return mixed
     */
    public function actionRequestPasswordReset()
    {
        $model = new PasswordResetRequestForm();
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail()) {
                Yii::$app->session->setFlash('success', 'Check your email for further instructions.');

                return $this->goHome();
            } else {
                Yii::$app->session->setFlash('error', 'Sorry, we are unable to reset password for email provided.');
            }
        }

        return $this->render('requestPasswordResetToken', [
            'model' => $model,
        ]);
    }

    /**
     * Resets password.
     *
     * @param string $token
     * @return mixed
     * @throws BadRequestHttpException
     */
    public function actionResetPassword($token)
    {
        try {
            $model = new ResetPasswordForm($token);
        } catch (InvalidParamException $e) {
            throw new BadRequestHttpException($e->getMessage());
        }

        if ($model->load(Yii::$app->request->post()) && $model->validate() && $model->resetPassword()) {
            Yii::$app->session->setFlash('success', 'New password was saved.');

            return $this->goHome();
        }

        return $this->render('resetPassword', [
            'model' => $model,
        ]);
    }
}
